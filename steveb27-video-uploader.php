...
define('STEVEB27_VU_VERSION', '1.1.1');
define('STEVEB27_VU_PLUGIN_FILE', __FILE__);
define('STEVEB27_VU_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('STEVEB27_VU_PLUGIN_URL', plugin_dir_url(__FILE__));
define('STEVEB27_VU_REVIEW_URL', 'https://wordpress.org/support/plugin/steveb27-video-uploader/reviews/#new-post');
define('STEVEB27_VU_AUTHOR_URL', 'https://steveb27.com');
...
class SteveB27_Video_Uploader {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_steveb27_vu_dismiss_review_notice', array($this, 'ajax_dismiss_review_notice'));
        add_filter('plugin_action_links_' . plugin_basename(STEVEB27_VU_PLUGIN_FILE), array($this, 'add_plugin_action_links'));
    }
    ...
    private function generate_video_html($url, $id = 0, $class = '') {
        ...
        return sprintf(
            '<div class="steveb27-video-container" data-video-id="%d">
                <video %s %s controls muted autoplay loop playsinline controlslist="nodownload">
                    <source src="%s" type="video/mp4">
                    %s
                </video>
                <button class="steveb27-fullscreen-btn" aria-label="%s">
    ...
    public function woocommerce_video_thumbnail($html, $attachment_id) {
        ...
        if ($post && strpos($post->post_mime_type, 'video/') === 0) {
            ...
            // We also add data-steveb27-video so our frontend JS can disable the
            // photoswipe lightbox on this item (lightbox can't display video).
            return sprintf(
                '<div data-thumb="%s" data-thumb-alt="%s" class="woocommerce-product-gallery__image" data-steveb27-video="1">
                    %s
                </div>',
                esc_url($thumb_url),
                $title,
    ...
    private function get_video_poster_url($attachment_id) {
        ...
        if ($thumb_id) {
            $thumb_arr = wp_get_attachment_image_src($thumb_id, 'thumbnail');
            if ($thumb_arr && isset($thumb_arr[0])) {
                return $thumb_arr[0];
            }
        }
    ...
    /**
     * Provide a fallback image src for videos so they appear in contexts expecting
     * an <img> (like Elementor, Divi builders, or any code that only checks for images).
    ...
    public function video_attachment_image_src($image, $attachment_id, $size, $icon) {
        ...
        if ($post && strpos($post->post_mime_type, 'video/') === 0) {
            ...
            return array($placeholder, 150, 150, false);
        }
        return $image;
    }

    private function get_video_placeholder_url() {
        return STEVEB27_VU_PLUGIN_URL . 'img/video-placeholder.svg';
    }
    ...
    public function enqueue_admin_scripts($hook) {
        if (in_array($hook, array('post.php', 'post-new.php', 'upload.php'), true)) {
            wp_enqueue_script(
                'steveb27-vu-admin',
                STEVEB27_VU_PLUGIN_URL . 'js/admin.js',
                array('jquery'),
                STEVEB27_VU_VERSION,
                true
            );
        ...
        wp_enqueue_style('steveb27-vu-admin', STEVEB27_VU_PLUGIN_URL . 'css/admin.css', array(), STEVEB27_VU_VERSION);

        // Register the notice handler script
        wp_register_script(
            'steveb27-vu-notice-handler',
            '', // Empty string for inline script
            array('jquery'),
            STEVEB27_VU_VERSION,
            true
        );

        // Provide AJAX URL + nonce for notice dismissal
        wp_localize_script('steveb27-vu-notice-handler', 'steveb27VuAdmin', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce'   => wp_create_nonce('steveb27_vu_dismiss_review_notice'),
        ));
    }

    public function enqueue_frontend_scripts() {
        wp_enqueue_style('steveb27-vu-frontend', STEVEB27_VU_PLUGIN_URL . 'css/frontend.css', array(), STEVEB27_VU_VERSION);
        wp_enqueue_script('steveb27-vu-frontend', STEVEB27_VU_PLUGIN_URL . 'js/frontend.js', array('jquery'), STEVEB27_VU_VERSION, true);
    }
    ...
    public function maybe_show_review_notice() {
        ...
        if (get_user_meta($user_id, 'steveb27_vu_review_notice_dismissed', true)) {
            return;
        }
        $activated = get_option('steveb27_vu_activation_time');
        if (!$activated || (time() - intval($activated)) < (7 * DAY_IN_SECONDS)) {
            return;
        }

        // Enqueue the notice handler script
        wp_enqueue_script('steveb27-vu-notice-handler');

        // Add the inline script for notice dismissal
        $inline_script = "
        (function($) {
            $('#steveb27-vu-review-notice').on('click', '[data-steveb27-action]', function(e) {
                var \$btn   = $(this);
                var action = \$btn.data('steveb27-action');
                // Only prevent default for non-link actions or internal anchors
                if (\$btn.attr('href') === '#' || !\$btn.attr('href')) {
                    e.preventDefault();
                }
                $.post(steveb27VuAdmin.ajaxUrl, {
                    action: 'steveb27_vu_dismiss_review_notice',
                    nonce:  steveb27VuAdmin.nonce,
                    dismissal_type: action
                });
                $('#steveb27-vu-review-notice').fadeOut(300, function() { $(this).remove(); });
            });
        })(jQuery);
        ";
        wp_add_inline_script('steveb27-vu-notice-handler', $inline_script);

        ?>
        <div class="notice steveb27-vu-review-notice" id="steveb27-vu-review-notice">
            <div class="steveb27-vu-review-notice__inner">
                <div class="steveb27-vu-review-notice__icon">🎬</div>
                <div class="steveb27-vu-review-notice__content">
                    <p>
                    ...
                    <p class="steveb27-vu-review-notice__actions">
                        <a href="<?php echo esc_url(STEVEB27_VU_REVIEW_URL); ?>" target="_blank" rel="noopener noreferrer"
                           class="button button-primary steveb27-vu-review-btn" data-steveb27-action="reviewed">
                            ⭐ <?php esc_html_e('Leave a Review', 'steveb27-video-uploader'); ?>
                        </a>
                        <a href="#" class="button steveb27-vu-review-btn" data-steveb27-action="reviewed">
                            <?php esc_html_e('I already did!', 'steveb27-video-uploader'); ?>
                        </a>
                        <a href="#" class="button-link steveb27-vu-review-later" data-steveb27-action="later">
    ...
    public function ajax_dismiss_review_notice() {
        check_ajax_referer('steveb27_vu_dismiss_review_notice', 'nonce');
        if (!current_user_can('manage_options')) {
            wp_die();
        }
        ...
        if ('later' === $type) {
            ...
            update_option('steveb27_vu_activation_time', time());
        } else {
            update_user_meta($user_id, 'steveb27_vu_review_notice_dismissed', '1');
        }
        wp_die();
    }
    ...
    public function add_plugin_action_links($links) {
        $extra = array(
            '<a href="' . esc_url(STEVEB27_VU_REVIEW_URL) . '" target="_blank" rel="noopener noreferrer">'
            . esc_html__('Leave a Review', 'steveb27-video-uploader') . '</a>',
            '<a href="' . esc_url(STEVEB27_VU_AUTHOR_URL) . '" target="_blank" rel="noopener noreferrer">'
...
function steveb27_vu_on_activation() {
    ...
    add_option('steveb27_vu_activation_time', time());
}
register_activation_hook(STEVEB27_VU_PLUGIN_FILE, 'steveb27_vu_on_activation');

function steveb27_vu_on_deactivation() {
    delete_option('steveb27_vu_activation_time');
}
register_deactivation_hook(STEVEB27_VU_PLUGIN_FILE, 'steveb27_vu_on_deactivation');

// -------------------------------------------------------------------------
// Bootstrap
// -------------------------------------------------------------------------

function steveb27_video_uploader_init_instance() {
    return SteveB27_Video_Uploader::get_instance();
}
add_action('plugins_loaded', 'steveb27_video_uploader_init_instance');